...
import IntermediateTerm.IOrTerm;
...
public class Or extends PCFTerm {
    ...
    public String toStringPrec(int precedence) {
        String result = left.toStringPrec(4) + " or " + right.toStringPrec(this.precedence);
    ...
    protected void type(VariableGenerator gen, Map<String, TypeRef> env, Program prog) throws TypingException {
        ...
        try {
            ...
            try {
                ...
                throw new TypingException("Cannot unify " + right + " with Bool in Or.");
            }
        } catch (UnificationException e) {
            throw new TypingException("Cannot unify " + left + " with Bool in Or.");
        }
    }
	...
	public PCFTerm substitute(String x, PCFTerm prog) {
    	return new Or(left.substitute(x, prog), right.substitute(x, prog));  
	}
    ...
    public PCFTerm execute(Program prog) {
        ...
        if((updatedLeft instanceof False) && (updatedRight instanceof False)) {
            return new False();
        } 
	...
	public IntermediateTerm toIntermediate() {
		return new IApplication(new IApplication(new IOrTerm(),left.toIntermediate()), right.toIntermediate()); 
	}
