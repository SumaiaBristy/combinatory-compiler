...
import IntermediateTerm.ILEqualTerm;
...
public class LEqual extends PCFTerm {
    ...
    public String toStringPrec(int precedence) {
        String result = left.toStringPrec(10) + " <= " + right.toStringPrec(this.precedence);
	...
	public PCFTerm substitute(String x, PCFTerm prog) {
		return new LEqual(left.substitute(x, prog), right.substitute(x, prog));  
	}
    ...
    public PCFTerm execute(Program prog) {
        ...
        if((updatedLeft instanceof IntLiteral lnum) && (updatedRight instanceof IntLiteral rnum)) {
            ...
            return new LEqual(updatedLeft, updatedRight);
        }    
    }

    @Override
    public IntermediateTerm toIntermediate() {
        return new IApplication(new IApplication(new ILEqualTerm(),left.toIntermediate()), right.toIntermediate()); 
    }
}
